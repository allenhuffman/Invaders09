...
*
* Invaders.asm V0.06 by Allen C. Huffman
...
****Allen, If you don't want to use os9defs.a remove the next three
****lines. I think it is easier than using sys.l with rlink.
* ifp1
* use /dd/defs/os9defs.a
* endc
****Robert, I don't understand how to compile without using rlink... - Allen
...
score rmb  5        actual score
dlives rmb 9        lives esc sequence
lives rmb 2         player's lives
dlevel rmb 10       ditto for level (round)
level rmb 1
...
buffer rmb 2        misc. buffer stuff
pid   rmb 2         place to store process ID and other byte
tmode rmb 32        place for tmode settings

montype rmb 1       option flags
systype rmb 1
zoomode rmb 1
...
start
 ...
 clr <systype       init system type (0=Stock OS-9)
...
option
 ...
 bra option
optskip
 cmpa #'n           is it -n = NitrOS-9?
 bne optskip2       no, skip
 sta <systype       store system type
 bra option         go back and check for more
optskip2
 cmpa #'z           is it -z = Zoom Mode?
 bne optskip3       no, skip
 sta <zoomode
 bra option
optskip3
...
continue
 ...
 clrb               ss.opt getstat
 leax >tmode,u      point to tmode storage area
...
continue2
 ...
 lda <systype       check system type
 bne nitrostart     if not 0, skip to nitro method
stockstart
...
* Screen has been mapped in.  Ready to proceed.
 ...
 leax -36,x         go back to 1 right of center
...
gamestart
 ...
 leay >dscore,u
 lda #scorelen+liveslen+levellen
initit ldb ,x+
 stb ,y+
 deca
 bne initit
...
einit ldd ,x++      grab initial location
 ...
 suba <round        subtract round from it
 sta <edelay
 sta <ecount
...
bombdrop
* I need to add some kinda delay in here...
 dec <bcount        decrement bomb counter
 bne bombskip
 lda <bdelay
 sta <bcount
 pshs x,y
 leay enemies+(ENEMIES-4)*2,u
 ldb #ENEMIES
droploop
...
* Check for keyboard stuff.
 ...
 lbne moveright
 lbra shutdown       neither, so quit.

fire dec <firecount decrement fire counter
 lda <firecount
 anda #%00000011
 lbne moveskip
 lda #MISSILES      find available missile
...
shutdown
 ...
 clra               0
 sta <pid+1         group 0 means bye bye fonts
 lda <path
 leax >buffer,u
 ldy #4
...
getkey lda <path    read one key from path
 ldb #1
 os9 I$GetStt       check to see if character was in buffer
 bcs getit          if not, go wait for one
...
helpmsg fcb $d,$a
 fcc "Invaders 0.06 by Allen Huffman of Sub-Etha Software (coco-sysop@genie.geis.com)"
 fcb $d,$a
 fcc "Use LEFT/RIGHT to Move, SPACE to Fire, UP/DOWN/CTRL/ALT to Quit."
 fcb $d,$a,$d,$a
 fcc "Syntax: Invaders [-opts]"
 fcb $d,$a
 fcc "Usage : Silly little space game."
 fcb $d,$a
 fcc "Opts  : -? = display this message."
 fcb $d,$a
 fcc "        -m = monochrome colors (for 'montype m' displays)."
 fcb $d,$a
 fcc "        -n = use NitrOS-9 system call to allocate screen."
 fcb $d,$a
 fcc "        -z = zoom mode."
 fcb $d,$a,$d,$a
 fcc "This is an ALPHA copy.  If you didn't get this from Allen . . ."
 fcb $d,$a
...
title 
 ...
 fcc "THIS IS AN ALPHA TEST COPY.  NOT FOR"
 fcb $2,$20+2,$20+17
 fcc "REDISTRIBUTION.  ALPHA TESTERS ONLY!"
titlelen equ *-title
